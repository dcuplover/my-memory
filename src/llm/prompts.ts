...
提取不出高质量三元组就不提取，返回空数组。垃圾三元组比空数组更有害。
以下特征的"实体"是垃圾，必须丢弃：
- **行为/动作描述**："设置环境变量"、"重启服务"、"用完即关模式"
- **现象/状态描述**："worker 进程无法获取锁"、"长期持有锁"、"连接超时"
- **解决方案描述**："短连接模式"、"分批处理策略" — 除非它是一个被广泛接受的具名模式
- **过于空泛的词**："数据"、"新数据"、"问题"、"方案"、"配置"、"错误"、"结果"

## 实体要求

实体必须是**可命名、可指代、有明确边界的事物**。如果你无法用一个专有名称或术语来称呼它，它就不是实体。

实体必须属于以下类型之一（entity_type 字段必填）：
- Person — 人名
- Software — 软件、库、框架、工具、产品（须有具体名称，如 KùzuDB、Redis）
- Parameter — 配置参数、环境变量、API 参数（须有具体参数名，如 maxDBSize、TIMEOUT）
- Platform — 平台、操作系统、运行环境（如 Linux、Docker、Node.js）
- Concept — **已命名的**技术概念、设计模式、方法论（如 mmap、连接池、MVCC、观察者模式）。注意：行为描述、现象描述、临时方案描述不是 Concept
- Organization — 公司、团队、社区
- Format — 文件格式、协议、标准（如 JSON、HTTP、OpenAPI）
- Language — 编程语言、自然语言
- Error — 已命名的错误类型或异常类（如 OOM、SegmentFault、ECONNREFUSED）
...
- ❌ 现象描述当实体："worker 进程无法获取锁" — 这是一个句子/现象，不是可命名的事物
- ❌ 方案描述当实体："用完即关模式"、"短连接模式" — 除非这是一个业界公认的具名模式
- ❌ 同义重复："短连接" vs "短连接模式"、"长期持有锁" vs "长期持有连接" — 要么归一为一个实体，要么都不提取

**自检规则**：对每个候选实体问自己——"这个词能作为百科词条的标题吗？" 如果不能，它大概率不是实体。

### 实体规范化：
- **大小写归一**：同一实体统一为最常见的大小写形式（"Python" 和 "python" → "Python"；"LanceDB" 和 "lancedb" → "LanceDB"）
- **去修饰词**：去掉临时性修饰，保留核心名词（"新数据" → 不提取；"KùzuDB 的写锁" → 实体是 "KùzuDB" 和 "写锁"，不是 "KùzuDB 的写锁"）
- **去冗余后缀**：如果 X 和 X模式/X方案/X策略 指代同一事物，只保留最简短的那个
- **版本号**：不带版本号，除非版本是核心区分信息（如 Python 2 vs Python 3）
...
输入陈述：
1. worker 进程在写入时会长期持有数据库连接，导致其他进程无法获取锁
2. 可以用短连接模式来避免锁冲突，即用完立即关闭连接
3. 新数据写入后需要等待一段时间才能被查询到

期望输出（大部分陈述不包含具名实体，宁可不提取）：
{
  "triples": []
}

分析：这些陈述描述的是现象和临时方案，缺乏具名实体。"worker 进程""新数据""短连接模式"都不是可命名的技术概念或产品。如果原文提到了具体的数据库名（如 KùzuDB），才值得提取。

## 输出格式

每条陈述可提取 0~3 个三元组。**提取不出高质量三元组就返回空数组，不要凑数。**
仅输出 JSON：
{
  "triples": [
